...
public class InputParaMapReduce {
	...
	private static InputParaMapReduce getInputParaMapReduceInstance=null;
	
	private InputParaMapReduce() {}	
	
	public static InputParaMapReduce getInstance(){
		if(getInputParaMapReduceInstance==null){
			getInputParaMapReduceInstance=new InputParaMapReduce();
		}
		return getInputParaMapReduceInstance;
	}

	
	private static class findInputParaMapper extends Mapper<Edge, DoubleWritable, Text, DoubleWritable> {
	...
	private static class findInputParaReducer extends Reducer<Text, DoubleWritable, Text, DoubleWritable> {
	...
	private boolean findInputPara(Configuration conf, String input,
	    ...
	    FileInputFormat.addInputPath(findInputParaJob, new Path(input));
	    FileOutputFormat.setOutputPath(findInputParaJob, new Path(output));
	    
	    return findInputParaJob.waitForCompletion(true);		
		
	}
	
	public void getSimiForFindInput(Configuration conf, String inputFile, String outputFile) throws Exception {
		findInputPara(conf,inputFile, outputFile);	
